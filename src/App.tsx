...
function App() {
	...
	const deletePageRecursive = (
		pages: Page[],
		pageId: number,
	): [Page[], Page | null] => {
		let updatedParent: Page | null = null;
		let indexOfDeletedPage: number | null = null;

		const updatedPages = pages.reduce((acc, page, index) => {
			if (page.id === pageId) {
				indexOfDeletedPage = index - 1;
				if (indexOfDeletedPage >= 0) {
					updatedParent = acc[indexOfDeletedPage];
				}
				return acc;
			}

			if (page.child && page.child.length > 0) {
				const [updatedChild, newParent] = deletePageRecursive(
					page.child,
					pageId,
				);
				page.child = updatedChild;
				if (newParent) {
		...
		return [updatedPages, updatedParent];
	};

	const deletePage = (pageId: number): void => {
		setPages((prevPages) => {
			const [updatedPages, newActivePage] = deletePageRecursive(
				prevPages,
				pageId,
			);
			if (newActivePage) {
